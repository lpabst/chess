...
class Square extends Component {
  render() {
    ...
    let { piece, selectedPiece } = this.props;
    let squareBorder;
    let offendingPiece;
    ...
    for (let p = 0; p < this.props.errorPieceLocations.length; p++) {
      if (
        i === this.props.errorPieceLocations[p][0] &&
        j === this.props.errorPieceLocations[p][1]
      ) {
        offendingPiece = true;
      }
    }
    ...
    if (
      selectedPiece &&
      i === selectedPiece.location[0] &&
      j === selectedPiece.location[1]
    ) {
      ...
      const availableMoves =
        selectedPiece && selectedPiece.availableMoves
    ...
    let squareStyle = {
      //   background:
      // j % 2 === i % 2
      //   ? 'url("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQE6ctrqwxsbjV8wTdAL83MzpH6XoVfLfRYSXv8Lmf530pDjWrI")'
      //   : "url('https://4my3boyz.com/content/images/thumbs/0014032_danscapes-spring-wood-grain-dark-brown-cotton-fabric_500.jpeg')",
      background: j % 2 === i % 2 ? "#edc795" : "#916326",
      border: squareBorder,
    };
    let pieceStyle = piece
      ? {
          background:
            "url('/media/" + piece + ".png')center center /cover no-repeat",
    ...
    return (
      <div
        className="square"
        style={squareStyle}
        onClick={(e) => this.props.handleClick(i, j)}
      >
